...
import {
  ...
  getRFQTypeDisplay,
} from "../utils/rfq-status-helpers";
import {
  getDeadlineBadgeInfo,
  getRFQCardSummary,
} from "../utils/rfq-card-helpers";

interface SupplierRFQCardProps {
  rfq: RFQDto;
}
...
export const SupplierRFQCard: React.FC<SupplierRFQCardProps> = ({ rfq }) => {
  ...
  const handleCreateQuotation = (e: React.MouseEvent) => {
    e.stopPropagation();
    router.push(`/supply/supplier/quotations/add-edit/new?rfqId=${rfq.id}`);
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-16 p-md-20 d-flex flex-column flex-grow-1">
        ...
        {/* Created Date with Action Buttons */}
        {rfq.createdAt && (
          <div className="d-flex align-items-center justify-content-between gap-12 mb-12">
            ...
            {!isExpired && rfq.status === "PUBLISHED" && (
              <Button
                variant="success"
                size="xs"
                rightIcon="ph-bold ph-plus"
