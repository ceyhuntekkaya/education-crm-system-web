/**
 * Ürün görselleri için fallback/placeholder görseller
 * Görsel yüklenemediğinde rastgele bunlardan biri gösterilir
 */
...
/**
 * Rastgele bir placeholder görsel döndürür
 * Her görsel için tutarlı bir placeholder almak için index parametresi kullanılabilir
 * @param index - Opsiyonel index değeri (aynı index her zaman aynı görseli döndürür)
 * @returns Placeholder görsel URL'i
 */
export const getRandomPlaceholderImage = (index?: number): string => {
  if (index !== undefined) {
    // Index verilmişse, modulo ile tutarlı bir görsel seç
    return PRODUCT_PLACEHOLDER_IMAGES[
      index % PRODUCT_PLACEHOLDER_IMAGES.length
    ];
  }

  // Index verilmemişse, rastgele bir görsel seç
